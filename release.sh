...
#   3. Copies plugin to a clean temp dir; sets * Version + NERA_SL_VERSION; syncs readme Stable tag + plugin.json
...
echo "▶ Setting * Version + NERA_SL_VERSION to ${VERSION} in release tree (for PUC)..."
...
if sed --version >/dev/null 2>&1; then
  ...
  sed -i "s/define( 'NERA_SL_VERSION', '[^']*' );/define( 'NERA_SL_VERSION', '${VERSION}' );/" "$PHP_MAIN"
else
  sed -i '' "s/^ \\* Version: .*/ * Version: ${VERSION}/" "$PHP_MAIN"
  sed -i '' "s/define( 'NERA_SL_VERSION', '[^']*' );/define( 'NERA_SL_VERSION', '${VERSION}' );/" "$PHP_MAIN"
fi
